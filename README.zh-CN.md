...
在 **Agent** 工作区选择 **Tutti Agent**，然后点击 **连接**。Tutti 会自动检测或安装本地
`tutti-agent` 命令行适配器；如果页面提示登录，点击 **登录**，在浏览器完成 Tutti 账号登录后返回应用。
本地适配器和账号授权都准备完成后，状态会变为已连接。

这不是填写服务器地址或 API Key 的配置流程。Tutti Agent 通过本地
`tutti-agent app-server` 运行，Tutti 会管理本地适配器及其账号令牌。如果连接失败，请确认电脑可访问
npm registry 与 Tutti 账号登录页，然后再次点击 **连接**。
