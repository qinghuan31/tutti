...
Repository scanners that compare relative paths against POSIX-style package
fragments must normalize Windows `\\` separators to `/` before applying
inclusions or exemptions. Otherwise a valid package-local exemption can work in
CI on Linux or macOS while failing on Windows.
