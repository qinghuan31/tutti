...
`TUTTI_DESKTOP_MAC_ARCH` selects the Electron package architecture and accepts
only `x64`, `arm64`, or `universal`. The release workflow sets it for each
matrix entry; local release builds may set it explicitly when reproducing one
matrix lane. This variable does not change Windows packaging.
