...
`packages/agent/runtimeprep/preparer.go` 的 `NewDefaultPreparer` 注册 `CodexPreparer`（`codex.go`）、`ClaudeCodePreparer`、`CursorPreparer` 及三个 `InstructionFilePreparer`，接口为 `ProviderPreparer`（`Provider() string` + `Prepare(...) (ProviderPrepareResult{Cwd, Env}, error)`）。
...
共享 provider 准备位于 `packages/agent/runtimeprep/tutti_agent.go`（由 tuttid 注册进 `NewDefaultPreparer`），产品侧鉴权 bootstrap 位于 `services/tuttid/service/tuttiagent/service.go`。准备职责对照 `CodexPreparer`：
...
- 在 `packages/agent/runtimeprep/tutti_agent.go` 实现共享 preparer 并由 tuttid 注册；在 `services/tuttid/service/tuttiagent/service.go` 保留产品侧 token bootstrap。
