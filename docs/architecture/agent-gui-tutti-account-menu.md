...
- `UserInfo.is_vip`
- `UserInfo.vip_level`: `free | basic | pro | ultra`
- `UserInfo.vip_billing_period`: `month | year`
- `UserInfo.vip_renew_at`
- `UserInfo.vip_valid_until`
- `UserInfo.vip_cancel_at_period_end`
...
- AgentGUI 账号入口的会员展示应复用 `Basic / Pro / Ultra` 命名。
- `is_vip=false` 或 `vip_level=free` 时展示 `Free`。
...
- 副文案展示会员档位：`Basic / Pro / Ultra / Free`。
...
| 行       | 展示                                                      | 点击行为                           |
| -------- | --------------------------------------------------------- | ---------------------------------- |
| 会员     | `Basic / Pro / Ultra`，非会员为 `Free`，右侧可展示 `升级` | 打开 `/profile/plan`               |
...
- daemon 从 Commerce `user-info` 的 `is_vip` 和 `vip_level` 计算当前权益。
- `is_vip=false` 或 `vip_level=free`：对 desktop/AgentGUI 聚合响应返回 `membership = null`，展示 `Free`。
- `is_vip=true` 且 `vip_level` 为 `basic | pro | ultra`：返回对应 `membership` 摘要。
- `vip_renew_at` 表示自动续费时间；取消但权益仍可用时用 `vip_valid_until` 表示有效期截止。
...
- view 测试：
  ...
  - member summary 能渲染 `Basic / Pro / Ultra`。
...
- `basic` 会员在 AgentGUI 中显示为 `Basic`，与 `tsh-web` 保持一致。
- 非会员展示 `Free`，不是空会员状态。
