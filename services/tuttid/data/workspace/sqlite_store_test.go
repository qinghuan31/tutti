...
func TestSQLiteDSNUsesWindowsFileURLPath(t *testing.T) {
	if runtime.GOOS != "windows" {
		t.Skip("Windows SQLite file URL test")
	}

	dsn := sqliteWriterDSN(`C:\Users\Tutti User\state\tuttid.db`)
	if !strings.HasPrefix(dsn, "file:///C:/Users/Tutti%20User/state/tuttid.db?") {
		t.Fatalf("sqliteWriterDSN() = %q, want canonical Windows file URL", dsn)
	}
	if strings.Contains(dsn, "%5C") {
		t.Fatalf("sqliteWriterDSN() = %q, contains encoded Windows separators", dsn)
	}
}
