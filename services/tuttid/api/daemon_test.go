...
func TestDaemonAPIGeneratedRoutesUpdateAgentSessionTitle(t *testing.T) {
	...
	RegisterRoutes(mux, NewRoutes(DaemonAPI{
		AgentSessionService: stubAgentSessionService{
			updateTitleFn: func(_ context.Context, workspaceID string, agentSessionID string, title string) (agentservice.Session, error) {
				if workspaceID != "ws-1" {
					t.Fatalf("workspaceID = %q, want ws-1", workspaceID)
				}
				if agentSessionID != "session-1" {
				...
				if title != "Renamed session" {
					t.Fatalf("title = %q, want Renamed session", title)
				}
				return agentservice.Session{
					ID:        agentSessionID,
					Provider:  "codex",
	...
	recorder := performGeneratedRouteRequest(
		...
		map[string]any{"title": "Renamed session"},
	)
	if recorder.Code != http.StatusOK {
		t.Fatalf("status = %d, want %d; body: %s", recorder.Code, http.StatusOK, recorder.Body.String())
	}
	...
	var response tuttigenerated.WorkspaceAgentSessionResponse
	decodeGeneratedRouteResponse(t, recorder, &response)
	if response.Session.Title == nil || *response.Session.Title != "Renamed session" {
		t.Fatalf("response title = %#v, want Renamed session", response.Session.Title)
	}
}
