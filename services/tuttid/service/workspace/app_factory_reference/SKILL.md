...
Avoid startup-time package installation. If dependencies or build artifacts are necessary, add an executable `prepare.sh` and keep `bootstrap.sh` focused on launching the prepared app. Windows-compatible packages must add an equivalent sibling `prepare.cmd`; Tutti runs that companion on Windows. The preparation scripts may use `TUTTI_APP_PYTHON`, `TUTTI_APP_NODE`, and `TUTTI_APP_NPM` for install and build steps, using POSIX `$VAR` syntax in `.sh` and `%VAR%` syntax in `.cmd`.
