...
`), 0o755); err != nil {
	...
	if runtime.GOOS == "windows" {
		prepareCMDPath := filepath.Join(packageDir, "prepare.cmd")
		if err := os.WriteFile(prepareCMDPath, []byte("@echo off\r\necho %TUTTI_APP_TOOLCHAIN_ROOT%>\"%TUTTI_APP_DATA_DIR%\\toolchain-root.txt\"\r\n"), 0o644); err != nil {
			t.Fatalf("write prepare.cmd: %v", err)
		}
	}

	err := prepareAppFactoryJobWithRuntimeResolver(ctx, workspacebiz.AppFactoryJob{
		AppID:      "app_1",
		DraftDir:   draftDir,
		RuntimeDir: runtimeDir,
		DataDir:    dataDir,
		LogDir:     logDir,
	}, staticAppRuntimeResolver{})
	if err != nil {
...
func TestPrepareAppFactoryJobRequiresWindowsCompanion(t *testing.T) {
	if runtime.GOOS != "windows" {
		t.Skip("Windows prepare companion test")
	}
	...
	for _, dir := range []string{packageDir, logDir} {
		if err := os.MkdirAll(dir, 0o755); err != nil {
			t.Fatalf("create %s: %v", dir, err)
		}
	}
	if err := os.WriteFile(filepath.Join(packageDir, "prepare.sh"), []byte("#!/bin/sh\n"), 0o755); err != nil {
		t.Fatalf("write prepare.sh: %v", err)
	}

	err := prepareAppFactoryJobWithRuntimeResolver(context.Background(), workspacebiz.AppFactoryJob{
		DraftDir: draftDir,
		LogDir:   logDir,
	}, staticAppRuntimeResolver{})
	if err == nil || !strings.Contains(err.Error(), "prepare.cmd is required") {
		t.Fatalf("prepareAppFactoryJob() error = %v, want missing prepare.cmd error", err)
	}
}
