...
func TestDefaultRegistryUsesTuttiAgentManagedNPMInstaller(t *testing.T) {
	...
	wantVersion := tuttiAgentPackageVersionForOS(runtime.GOOS)
	if install.ManagedNPM.PackageVersion != wantVersion {
		t.Fatalf("PackageVersion = %q, want %q", install.ManagedNPM.PackageVersion, wantVersion)
	}
	...
	if specs[0].AdapterPackage.Name != "@tutti-os/tutti-agent" || specs[0].AdapterPackage.Version != wantVersion {
		t.Fatalf("AdapterPackage = %+v, want @tutti-os/tutti-agent >= %s", specs[0].AdapterPackage, wantVersion)
	}
...
func TestTuttiAgentPackageVersionForOS(t *testing.T) {
	t.Parallel()

	cases := map[string]string{
		"darwin":  minTuttiAgentVersion,
		"linux":   minTuttiAgentVersion,
		"windows": windowsTuttiAgentVersion,
	}
	for goos, want := range cases {
		if got := tuttiAgentPackageVersionForOS(goos); got != want {
			t.Errorf("tuttiAgentPackageVersionForOS(%q) = %q, want %q", goos, got, want)
		}
	}
}
...
func TestTuttiAgentAdapterVersionMismatchRequiresInstall(t *testing.T) {
	...
	writePackageManifest(t, packageDir, "@tutti-os/tutti-agent", "0.0.0")
	writeExecutable(t, binPath, "#!/bin/sh\nif [ \"$1\" = \"--version\" ]; then echo 'tutti-agent 0.0.0'; exit 0; fi\nexit 0\n")
	...
	if status.Adapter.Version != "0.0.0" {
		t.Fatalf("Adapter.Version = %q, want 0.0.0", status.Adapter.Version)
	}
	wantVersion := tuttiAgentPackageVersionForOS(runtime.GOOS)
	if status.Adapter.RequiredVersion != wantVersion {
		t.Fatalf("Adapter.RequiredVersion = %q, want %q", status.Adapter.RequiredVersion, wantVersion)
	}
	action := firstAction(t, status.Actions)
	if action.ID != ActionInstall {
		t.Fatalf("first action ID = %q, want %q", action.ID, ActionInstall)
...
func TestServiceListReportsCodexChecksVersionAndLastError(t *testing.T) {
	...
	linkTestExecutable(t, codexPath, visiblePath)
...
func TestServiceListRunsCodexLauncherWithManagedNodePath(t *testing.T) {
	...
	linkTestExecutable(t, codexPath, filepath.Join(binDir, "codex"))
	platformPath := requireTestCodexPlatformBinaryPath(t, pkgDir)
	writeExecutable(t, platformPath, "#!/bin/sh\nexit 0\n")
...
func TestServiceRunCodexInstallerReportsManagedNPMActiveAction(t *testing.T) {
	...
	service.InstallCommand = func(ctx context.Context, input InstallCommandInput) (InstallCommandResult, error) {
		...
		platformPath := requireTestCodexPlatformBinaryPath(t, pkgDir)
		writeExecutable(t, platformPath, "#!/bin/sh\nexit 0\n")
		linkTestExecutable(t, codexPath, filepath.Join(binDir, "codex-test"))
		return InstallCommandResult{ExitCode: 0, Stdout: "installed"}, nil
	}
...
func isTestExecutable(path string) bool {
	...
	return runtime.GOOS == "windows" || stat.Mode().Perm()&0111 != 0
}
...
func commandBinaryNameForTest(name string) string {
...
func linkTestExecutable(t *testing.T, target string, link string) {
	t.Helper()
	if runtime.GOOS == "windows" {
		if err := os.Link(target, link); err != nil {
			t.Fatalf("link test executable: %v", err)
		}
		return
	}
