...
func (s Service) statusForSpec(ctx context.Context, spec ProviderSpec, now time.Time) (status ProviderStatus) {
	...
	if !installed {
		...
		actions = appendInstallActionIfSupported(actions, spec)
...
func appendInstallActionIfSupported(actions []Action, spec ProviderSpec) []Action {
	if spec.Install.Kind == "" {
		return actions
	}
	return append(actions, daemonAction(ActionInstall))
}
