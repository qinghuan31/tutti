...
func newClaudeBinaryFixture(t *testing.T, extraEnv ...string) claudeBinaryFixture {
	...
	resolvedEntry := service.resolveClaudeSDKSidecarEntryPath()
	if resolvedEntry != entry {
		t.Fatalf("resolveClaudeSDKSidecarEntryPath() = %q, want %q", resolvedEntry, entry)
	}
	if _, err := resolveClaudeSDKPackageDir(bundleDir); err != nil {
		t.Fatalf("resolveClaudeSDKPackageDir(%q): %v", bundleDir, err)
	}
