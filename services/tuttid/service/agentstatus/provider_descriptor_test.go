...
func TestClaudeCodeStatusSpecComesFromProviderDescriptor(t *testing.T) {
	...
	spec, err := providerSpecFromDescriptorForOS(descriptor, "darwin")
	if err != nil {
		t.Fatalf("providerSpecFromDescriptorForOS() error = %v", err)
	}
...
func TestProviderStatusAdapterSuppressesBashInstallersOnWindows(t *testing.T) {
	for _, providerID := range []string{
		...
		descriptor, ok := providerregistry.Find(providerID)
		if !ok {
			t.Fatalf("provider descriptor %q missing", providerID)
		}
		spec, err := providerSpecFromDescriptorForOS(descriptor, "windows")
		if err != nil {
			t.Fatalf("providerSpecFromDescriptorForOS(%q) error = %v", providerID, err)
		}
		if spec.Install.Kind != "" {
			t.Errorf("provider %q Windows installer = %#v, want unavailable", providerID, spec.Install)
		}
	}
}
