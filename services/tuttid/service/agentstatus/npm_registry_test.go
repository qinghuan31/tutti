...
func TestShellCommandUsesNPMRecognizesPortableAndWindowsCommands(t *testing.T) {
	for _, command := range []string{
		"npm install -g openclaw",
		"npm.cmd install -g openclaw",
		`"C:\\Program Files\\nodejs\\npm.cmd" install -g openclaw`,
	} {
		if !shellCommandUsesNPM(command) {
			t.Errorf("shellCommandUsesNPM(%q) = false, want true", command)
		}
	}
	if shellCommandUsesNPM("pnpm add openclaw") {
		t.Fatal("shellCommandUsesNPM() matched pnpm")
	}
}
