...
func providerSpecFromDescriptor(descriptor providerregistry.ProviderDescriptor) (ProviderSpec, error) {
	return providerSpecFromDescriptorForOS(descriptor, runtime.GOOS)
}

func providerSpecFromDescriptorForOS(descriptor providerregistry.ProviderDescriptor, goos string) (ProviderSpec, error) {
	...
	applyProviderPlatformRequirements(&spec, goos)
	return spec, nil
}

func applyProviderPlatformRequirements(spec *ProviderSpec, goos string) {
	if goos == "windows" &&
		spec.Install.Kind == InstallerKindOfficialScript &&
		spec.Install.ScriptShell == "bash" {
		spec.Install = InstallerSpec{}
	}
	if spec.Provider != providerregistry.TuttiAgentProviderID || spec.Install.ManagedNPM == nil {
		return
	}
	version := tuttiAgentPackageVersionForOS(goos)
	spec.Install.ManagedNPM.PackageVersion = version
	spec.Install.DisplayCommand = fmt.Sprintf(
		"npm install -g %s@%s --include=optional",
		spec.Install.ManagedNPM.PackageName,
		version,
	)
	spec.AdapterPackage = AdapterPackageRequirement{
		Name:    spec.Install.ManagedNPM.PackageName,
		Version: version,
	}
}
