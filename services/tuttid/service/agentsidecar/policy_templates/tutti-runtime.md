...
| `mention://agent-session/<sessionId>?workspaceId=...`           | `$tutti-handoff` | `{{CLI_COMMAND}} agent session-summary --session-id <session-id> --json`                                                                         |
| `mention://agent-target/<targetId>?workspaceId=...`             | `$tutti-handoff` | use `agent`/`codex`/`claude` from intent: an instruction for the mentioned agent -> hand off, do not do it yourself; a question about it -> read |
