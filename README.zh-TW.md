...
在 **Agent** 工作區選擇 **Tutti Agent**，然後點擊 **連接**。Tutti 會自動偵測或安裝本機
`tutti-agent` 命令列介面卡；如果頁面提示登入，點擊 **登入**，在瀏覽器完成 Tutti 帳號登入後返回應用。
本機介面卡和帳號授權都準備完成後，狀態會變為已連接。

這不是填寫伺服器位址或 API Key 的設定流程。Tutti Agent 透過本機
`tutti-agent app-server` 執行，Tutti 會管理本機介面卡及其帳號權杖。如果連接失敗，請確認電腦可存取
npm registry 與 Tutti 帳號登入頁，然後再次點擊 **連接**。
