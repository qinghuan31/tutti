...
test("resolveGoCommand preserves Windows executables and shells only command shims", () => {
  assert.deepEqual(resolveGoCommand("go", "win32"), {
    command: "go",
    shell: false
  });
  assert.deepEqual(resolveGoCommand("C:\\Go\\bin\\go.exe", "win32"), {
    command: "C:\\Go\\bin\\go.exe",
    shell: false
  });
  assert.deepEqual(resolveGoCommand("C:\\tools\\go.cmd", "win32"), {
    command: "C:\\tools\\go.cmd",
    shell: true
  });
});
